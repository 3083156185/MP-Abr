...
img_folder = 'images2'
SRC_MODEL_PATH = "my_best_model.epoch{epoch:02d}-loss{val_loss:.2f}.hdf5"
...
class MovenetMPOpenvino:
    def __init__(self, input_src="fight2.mp4",
                 xml=DEFAULT_MODEL,
                 device="CPU",
                 tracking=False,
                 score_thresh=0.2,
        ...
        self.dict_id2skeleton = dict_id2skeleton
        self.dict_id2frame = {0: 0}
    ...
    def pd_postprocess(self, inference):
        ...
        for i in range(6):
            ...
            if score > self.score_thresh:
                ...
                # keypoints2 = keypoints.astype(np.int)
                # skeletons.append(keypoints2)
                bodies.append(body)
    ...
    def run(self):
        ...
        color_skeleton = (255, 180, 90)
        color_box = (0, 255, 255)
        model = models.load_model("my_best_model.epoch{epoch_02d}-loss{val_loss_.2f}.hdf5")
        incnt = 0
        count = 0
        frame_buffer = 0
        self.fps = FPS()
                ...
                for body in bodies:
                    ...
                    if (incnt % 3 == 0):
                        ...
                        if body:
                            ...
                            if (body.track_id in self.dict_id2skeleton):

                                if (len(self.dict_id2skeleton[body.track_id]) < 14):
                                    numbers2 = list(np.ravel(body.keypoints_norm))
                                    numbers3 = self.dict_id2skeleton[body.track_id]
                                    numbers3.append(list(np.ravel(body.keypoints_norm)))
                                    self.dict_id2skeleton[body.track_id] = numbers3

                                elif (len(self.dict_id2skeleton[body.track_id]) == 14):
                                    numbers2 = list(np.ravel(body.keypoints_norm))
                                    numbers3 = self.dict_id2skeleton[body.track_id]
                                    numbers3.append(numbers2)
                                    self.dict_id2skeleton[body.track_id] = numbers3

                                    lm_list = np.array(self.dict_id2skeleton[body.track_id])
                                    ...
                                    x = (body.xmin + body.xmax) // 2
                                    y = (body.ymin + body.ymax) // 2
                                    cv2.putText(frame, label, (x, y), cv2.FONT_HERSHEY_PLAIN, 4, color_box, 3)

                                    # print("14")

                                    self.dict_id2frame[body.track_id] = incnt
                                    # print(self.dict_id2frame)


                                elif (len(self.dict_id2skeleton[body.track_id]) == 15):
                                    del self.dict_id2skeleton[body.track_id][0]
                                    numbers2 = list(np.ravel(body.keypoints_norm))
                                    numbers3 = self.dict_id2skeleton[body.track_id]
                                    numbers3.append(numbers2)
                                    self.dict_id2skeleton[body.track_id] = numbers3
                                    if (incnt - self.dict_id2frame[body.track_id] >= 6):
                                        lm_list = np.array(self.dict_id2skeleton[body.track_id])
                                        lm_list = np.expand_dims(lm_list, axis=0)
                                        # print(lm_list.shape)
                                        results = model.predict(lm_list)
                                ...
                                self.dict_id2skeleton[body.track_id] = numbers3
                                # print("0")
                                # print(self.dict_id2skeleton[body.track_id])
...
if 1:
    ...
    # multiperson_classifier = MultiPersonClassifier(SRC_MODEL_PATH, LABELS)
    # fall1.mp4 ==0  fight1.mp4==1 stand1.mp4==2  walk1.mp4==3
    pd = MovenetMPOpenvino(input_src="fight5.mp4",
                           xml=DEFAULT_MODEL,
                           device="CPU",
                           tracking="iou",
                           score_thresh=0.2,
